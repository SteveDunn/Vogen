...
namespace Analyzer1
{
    ...
    public class Analyzer1CodeFixProvider : CodeFixProvider
    {
        ...
        private async Task<Document> MakeUppercaseAsync(Document document, TypeDeclarationSyntax typeDecl, CancellationToken cancellationToken)
        {
            ...
            var newMember = GenerateSetPropertyMethod();
            ...
            //var newWorkspace = workspace.TryApplyChanges(newRoot);
...
private Validation ValidateInput(string input)
{
    return input.Length > 0 ? Validation.Ok : Validation.Invalid(""!!"");
